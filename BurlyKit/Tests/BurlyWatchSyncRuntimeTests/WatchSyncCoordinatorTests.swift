...
struct WatchSyncCoordinatorTests {
    ...
    func deliveredAndAckedSessionIsPruned() throws {
        ...
        #expect(sent.count == 1)
        #expect(sent[0].id == session.id)
        let sessionEnvelope = try #require(BurlySyncEnvelope.decode(sent[0].envelope).envelope)
    ...
    func coldReopenRetainsOnlyUnackedOutboxEntry() throws {
        ...
        #expect(retried.map(\.id) == [pending.id])
    }
    ...
    func placeholderEmbeddingSortsByExerciseID() throws {
        ...
        let envelope = try #require(BurlySyncEnvelope.decode(retransmission[0].envelope).envelope)
